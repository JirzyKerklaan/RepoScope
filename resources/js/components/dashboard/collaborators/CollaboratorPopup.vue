        ...
        <div class="flex-1 grid grid-cols-2 gap-6 p-6 overflow-hidden">
            <div class="flex flex-col border border-slate-800 rounded-lg bg-slate-950/50 overflow-hidden">
                <div class="p-4 border-b border-slate-800 bg-slate-900/50">
                    <h3 class="text-slate-100">Saved Members ({{ members.length }})</h3>
                </div>
                <div class="flex-1 overflow-y-auto p-4 space-y-2">
                    ...
                    >
                        <div class="flex-1 min-w-0">
                            <p class="text-slate-100 truncate">{{ member.display_name }}</p>
                            <p class="text-sm text-slate-400 truncate">@{{ member.username }}</p>
                        </div>
                        <button
                            size="icon"
            ...
            <div class="flex flex-col border border-slate-800 rounded-lg bg-slate-950/50 overflow-hidden">
                ...
                <div class="flex-1 p-6 space-y-6 overflow-y-auto">
                    ...
                    <div class="flex gap-3 pt-4">
                        <button
                            v-if="step === 'name'"
                            class="btn hover:bg-accent hover:text-accent-foreground dark:hover:bg-accent/50 py-2 flex-1 text-slate-200 bg-slate-700 hover:text-slate-100 hover:bg-slate-800"
                            @click="goBack"
                        >
                            Back
                        </button>
                    ...
                    <div class="mt-auto pt-6" v-if="message.message">
                        <div class="border rounded-lg p-4" :class="message.status === 200 ? 'bg-green-500/50 border-green-700' : 'bg-red-500/50 border-red-700'">
...
const message = ref({message: null, status: null,});
            ...
            .then((response) => {
                ...
                if (response.data.status === 200) {
                    if (response.data.type === 'added') {
                        props.members.push(frequentUser.value)
                    }
